...
module Whitehall
  class PublishingApi
    class DraftEnvironmentPopulatorTest < ActiveSupport::TestCase
      ...
      test "default_draft_items returns an enumeration of all items which can be sent to publishing api" do
        ...
        assert_equal expected_values, DraftEnvironmentPopulator.default_draft_items.to_a
      end

      test "default_draft_items returns only the latest edition of a document" do
        published_edition = create(:published_edition)
        latest_edition = create(:draft_edition, document: published_edition.document)

        assert_equal [latest_edition], DraftEnvironmentPopulator.default_draft_items.to_a
      end
    end
  end
end
